...
def getRepeatedness(graph, l_max = 5):
    """
    Calculate repeatedness of the graph.
    1) Find the eigenvector centrality of each nodes
    2) Compute the probability of paths of various lengths starting from each node
    3) For fixed path length, calculate th probabilty of paths
    Parameter:
    graph (nx.graph): the network
    l_max (int): max path length to consider
    Return:
    (dict): path length as key, variance as value
    """
    ...
    for u in graph.nodes():
        path_prob.append([[u], u, initial_prob[u]])

    variance = {}
    for l in range(l_max+1):
        probs, t_path_prob = [], []
        
        # construct paths of length l+1 from l
        for p in path_prob:
            u, v = p[1], p[2]

            for w in graph.neighbors(u):
                probs.append(p * edge_weight[(u,w)])
                t_path_prob.append([p[0] + [w], w, probs[-1]])
        variance[l] = np.var(probs)
        path_prob = t_path_prob
...
def generateFeatures(graph, label):
    ...
    repeatedness = getRepeatedness(graph)
    ...
    data = [abruptness,
            branching,
            repeatedness[5],            # save repeatedness for l = 5. change for other values
            melodic,
            pitch_range_rule,
            pitch_range_piece,
            pitch_change,
